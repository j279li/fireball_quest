      ...
      <div class="grid gap-6 md:grid-cols-2">
        {#each allCampaigns as c}
          <Card class="bg-slate-800 border-slate-700 hover:bg-slate-750 cursor-pointer">
            ...
            <CardFooter>
              <button class="w-full inline-flex items-center justify-center gap-2 rounded-md bg-sky-600 px-3 py-2 text-sm font-medium text-slate-50" on:click={() => goto(`/chat/${c.id}`)}>
                <Play class="w-4 h-4" />
                <span>Open Chat</span>
              </button>
            </CardFooter>
